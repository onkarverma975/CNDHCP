...
class IPCalculator(object):
    def __init__(self, ip_address, cdir=24):
        ...
        self.broadcastaddr = None

    def __repr__(self):
        #print "Calculating the IP range of %s/%s" % (".".join(map(str, self._address)), self._cidr)
        #print "=================================="
        print "IP Address with Subnet %s/%s" % (".".join(map(str, self._address)), self._cidr)
        print "Netmask %s" % (".".join(map(str, self.net_mask())))
        print "Network Address %s" % (".".join(map(str, self.network_ip())))
        print "Broadcast address %s" % (".".join(map(str, self.broadcast_ip())))
